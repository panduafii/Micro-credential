...
class AssessmentQuestionSnapshot(Base):
    ...
    question_template_id: Mapped[int | None] = mapped_column(ForeignKey("question_templates.id", ondelete="SET NULL"))
    sequence: Mapped[int] = mapped_column(Integer, nullable=False)
    question_type: Mapped[QuestionType] = mapped_column(Enum(QuestionType), nullable=False)
    prompt: Mapped[str] = mapped_column(Text, nullable=False)
    metadata: Mapped[dict | None] = mapped_column(JSON)
    ...
    responses: Mapped[list["AssessmentResponse"]] = relationship(back_populates="question_snapshot", cascade="all,delete-orphan")
...
class AssessmentResponse(Base):
    ...
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    assessment_id: Mapped[str] = mapped_column(ForeignKey("assessments.id", ondelete="CASCADE"), nullable=False, index=True)
    question_snapshot_id: Mapped[str] = mapped_column(ForeignKey("assessment_question_snapshots.id", ondelete="CASCADE"), nullable=False)
